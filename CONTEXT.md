...
- **Deliverables Manifest**: Formal output payload generated upon run completion (`release_video_mp4`, `mastered_soundtrack_wav`, `dialogue_bus_wav`, `subtitles_vtt`, `subtitles_srt`, and `deliverables.json`) with SHA-256 verification checksums.
- **Dynamic Track Manifest (`AudioTrackOption`)**: Typed audio stream configuration defining video container, synchronized audio stem, soft subtitle tracks (.vtt/.srt), neural voice attributions, and quality ratings.
- **Stem Auditioning**: Interactive multi-channel audio inspection (`StemTrackType: 'dialogue' | 'background' | 'master'`) allowing audio engineers to audition isolated dialogue dubs, M&E background stems, or final composite mixdowns.
- **Temporal Invariant / Timestamp Continuity**: The domain invariant requiring that when switching audio channels or stems during auditioning, the playhead position (`currentTime`) is strictly preserved, preventing disorientation and playhead resets.
- **HTTP 206 Streaming Bridge**: Partial content byte-range streaming bridge (`/api/v1/clips/preview-stream`) enabling low-latency cinema video and stem playback without full file buffering.
...
- **Atomic Manifest Persistence**: Disk-backed state persistence pattern (`_atomic_write_json` using `.tmp.json` + `os.replace` / `MoveFileExW`) guaranteeing zero partial-write corruption and Windows `[WinError 32]` lock resilience across `transcript.json`, `transcript_{lang}.json`, and `stems.json`.
- **Stem Timeline Invariant (`stems.json`)**: Structured audio stem manifest capturing exact millisecond dialogue timing boundaries (`start_s`, `end_s`, `target_duration_s`, `audio_path`, `voice_id`) with integer `segment_id` indexing, eliminating downstream desync during retries.
...
- **Execution Scaffolding**: Video & audio extraction via `ffmpeg`, direct zero-copy 16kHz audio pass-through in `DenoiseStage` (Demucs decommissioned for 1.8GB VRAM savings), windowed `AudioChunker` with Faster-Whisper ASR transcription (`transcription_checkpoint.json` persistent resumption), Silero VAD, and subtitle formatter.
- **Autonomous Multi-Agent Crew (`backend/app/agents/` & `frontend/lib/agents/`)**:
  ...
  - `VoiceDirector`: Neural voice selection & segment speech audio synthesis via pluggable synthesis adapters (`KokoroTTSAdapter` local 24kHz neural engine, `EdgeTTSAdapter` for 300+ Microsoft neural voices, `MockAudioAdapter` - TICKET-12, TICKET-25, TICKET-33).
...
- **Post-QA Acoustic Master Mixdown & Sidechain Bus Integration (TICKET-17, TICKET-35)**: Post-QA timeline positioning (`adelay`), scalable dialogue bus compositing (`amix` via isolated `-filter_complex_script`), dynamic -6dB M&E sidechain ducking, and EBU R128 (-24 LUFS) broadcast loudness mastering.
...
- **Dynamic Frontend Video Player & Demucs Stem Mixer Integration**: Full hydration of deliverables into `MultiAudioPlayer` with real release video and dubbed soundtracks, replacing hardcoded clips. Interactive `BeforeAfterPlayer` stem auditioning with gapless timestamp preservation across A/B channel toggles.
...
- **Testing**: 126 Pytest tests passing (backend) and 117 Vitest tests across 21 test files passing 100% (frontend).
...
## 6. Key Decisions Already Made (ADRs)
...
- **ADR-008**: Route all video and audio playback through `/api/v1/clips/preview-stream` with HTTP 206 Partial Content byte ranges, avoiding full-file buffering in memory and ensuring rapid seeking on large broadcast files.
- **ADR-009**: Maintain a strict playhead temporal invariant across stem channel switches in `BeforeAfterPlayer`, preserving `currentTime` during track toggles via native HTML5 `<audio>` element re-binding without pulling in heavyweight Web Audio graph dependencies (`/ponytail`).
- **ADR-010**: Extract media track composition, language metadata mapping, and GPU stage rules into a pure, framework-agnostic helper layer (`frontend/lib/mediaTrackHelpers.ts`) with dedicated unit tests, adhering strictly to the Single Responsibility Principle (`/clean-code`).
- **ADR-011**: Standardize Neural Speech Synthesis on local Kokoro-82M (StyleTTS 2 @ 24kHz PCM_16) with centralized language specifications (`backend/app/core/languages.py`) and automatic fallback to Microsoft Edge-TTS on unsupported locales or missing dependencies.
- **ADR-012**: Use Atomic JSON Disk Manifests (`transcript.json`, `stems.json`) and integer `segment_id` map indexing for cross-stage artifact rehydration, eliminating in-memory state loss and index desync on retried pipeline stages.
- **ADR-013**: Use Single-Flight Stage Mutexes (`_active_stage_locks`) in `RunExecutor` with automated run-completion cleanup to serialize concurrent execution requests and eliminate Windows file sharing collisions (`[WinError 32]`).
- **ADR-014**: Route multitrack audio compositing filtergraphs through temporary `-filter_complex_script` text files with strict UTF-8/LF line endings and UUID isolation, eliminating Windows `CreateProcess` 8,191-character buffer overflow limits on 150+ stems.
