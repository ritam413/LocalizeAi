...
with sync_playwright() as p:
    ...
    ctx = browser.new_context(viewport={"width":1920,"height":1080},
        record_video_dir=DEMO+"/videos-free", record_video_size={"width":1920,"height":1080})
    page = ctx.new_page()
    page.goto(BASE+"/crv-out/viewer.html")
    page.wait_for_timeout(2500)
    # slow scroll through frame wall + transcript, ~12s
    smooth_scroll(page, 2600, 60, 200)   # 12s
    page.wait_for_timeout(1500)
    video = page.video
    ctx.close()
    shutil.move(video.path(), DEMO+"/free-viewer.webm")
    print("free-viewer.webm done")

    # ---- 2. pro viewer ----
    ctx = browser.new_context(viewport={"width":1920,"height":1080},
        record_video_dir=DEMO+"/videos-pro", record_video_size={"width":1920,"height":1080})
    page = ctx.new_page()
    page.goto(BASE+"/crv-pro-out/viewer.html")
    ...
    page.evaluate("var v=document.getElementById('v'); v.currentTime=0; v.play()")
